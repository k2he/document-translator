#!/usr/bin/env python3
"""Generate translated_segments.json with English translations."""
import json
from pathlib import Path

# Load extracted segments
work_dir = Path(__file__).parent.parent / "work"
with open(work_dir / "extracted_segments.json", "r", encoding="utf-8") as f:
...
trans = {
    ...
    ",努力搞清楚了各种函数的小趋势导数的求": ", working hard to understand the methods for finding derivatives",
    "法": "",
    "这": "These",
    ...
    "再论极限": "More on Limits",
    "罗必塔法则(L": "L'Hopital's Rule (L",
    "罗必塔法则": "L'Hopital's Rule",
    ...
    ",感觉无从下手": ", it feels like there is no way to start",
    "是对的。": "is correct.",
    "正好比": "It is just like",
    ...
    "钱": "money",
    ",到底谁更牛、笑到最后": ", so who is really better and will have the last laugh",
    ",真是不好判断。": ", is really hard to judge.",
    "让我们再开动一下脑子想想": "Let us think a bit more about this",
    ...
    "是 ": "is ",
    "细想一下不难得出结论": "With a bit of thought, it is not hard to conclude",
    ...
    ",不就是变化率吗。": ", is it not just rate of change.",
    ...
    "已经在被法国数学家罗必塔": "was already proven by the French mathematician L'Hopital",
    ...
    "1):求": "1): Find",
    "解": "Solution",
    ":根据罗必塔法则": ": By L'Hopital's Rule",
    ",我们有": ", we have",
    "2):求 ": "2): Find ",
    ":用罗必塔法则": ": Using L'Hopital's Rule",
    ",得到": ", we get",
    "好玩吧?": "Fun, right?",
    "再看一下这两个极限": "Let us look at these two limits",
    "3):求": "3): Find",
    ...
    "中的": "in the",
    ", 结果不会变": ", the result will not change",
    ",都是 ": ", it is all ",
    ...
    "这些地": "these",
    "市级的 ": "municipal-level ",
    "没有可比性": "there is no comparison",
    "。我们再看一下地市级的 ": ". Let us look at municipal-level ",
    "遇": "meeting",
    "上 ": " ",
    ...
    "有了罗必塔法则在手": "With L'Hopital's Rule in hand",
    ",再来看我们在第二章要求您记住的两个著名": ", let us revisit the two famous",
    "极限": "limits",
    ",将": ", which will",
    ...
    "玩过的": "the things you have learned",
    "东东看得越清楚。": "become clearer.",
    "让我们看看导数还有啥用。": "Let us see what else derivatives are good for.",
    ...
    ", 自然引起了人": ", naturally attract peoples",
    ...
    "。先厘清几个关于极值问题的概念": ". Let us first clarify some concepts about extreme values",
    ...
    "有局部就有全局。": "Where there is local, there is global.",
    "如果对所有的 ": "If for all ",
    "那么 f(": "Then f(",
    "那么 ": "Then ",
    ...
    "接着聊一": "Let us continue discussing",
    "下": "",
    "对": "our",
    "两": "of",
    "个": "these",
    ...
    ",如果把地球上的海拔高度做为函数": ", if we consider Earths elevation as a function",
    ...
    "不管进步到啥": "No matter how far you have",
    "程度": "advanced",
    ...
    ",右边的函数定义区间不": ", the right functions domain is not",
    "封闭。": "closed.",
    ...
    ",总算是找到了一点求极值的道道了!": ", we have finally found some methods for finding extreme values!",
    ",找出": ", find",
    "函数的临界": "the functions critical",
    "点,也就是那些不存在导数或者导数为 ": "points, that is, those points where the derivative does not exist or equals ",
    "的": "",
    ";其次": "; second",
    ...
    ",如何判断我们辛苦找出来的函数的临界点是局部最大值": ", how do we determine whether the critical points we have worked hard to find are local maxima",
    ",还": ", or",
    "是局部最": "local mini",
    "小值呢?": "ma?",
    ...
    "函": "the",
    "数的在一点的导数值的正负": "sign of a functions derivative at a point",
    ",决定着函数在此点的上升": ", determines whether the function is increasing",
    "或者下降的小趋势。": "or decreasing at that point.",
    "由": "From",
    "此不难推出": "this, it is not hard to deduce",
    ",如果在某个区间内": ", if on some interval",
    "函数的导数为正": "the functions derivative is positive",
...
for seg in segments:
    source = seg["source"]
    translation = trans.get(source, source)  # Use translation if exists, else keep source
    result.append({
        "id": seg["id"],
        "source": source,
...
translated_count = sum(1 for r in result if r["translation"] != r["source"])
print(f"Translated {translated_count}/{len(result)} segments")

with open(work_dir / "translated_segments.json", "w", encoding="utf-8") as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print(f"Created {work_dir / 'translated_segments.json'}")
